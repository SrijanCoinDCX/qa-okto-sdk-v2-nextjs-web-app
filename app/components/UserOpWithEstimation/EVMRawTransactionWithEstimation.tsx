...
import { ChevronDown, ArrowRight, RefreshCw, Check, AlertCircle, Copy, ExternalLink, Zap, Shield, Cpu } from 'lucide-react';
import {
    getChains,
    useOkto,
    UserOp,
...
const EVMRawTransactionEstimate = () => {
    ...
    const validateForm = () => {
        ...
        if (!formData.toAddress?.startsWith('0x')) return 'Invalid recipient address';
        if (formData.value && isNaN(Number(formData.value))) return 'Invalid value';
        if (formData.transactionData && !formData.transactionData.startsWith('0x')) {
            return 'Transaction data must start with 0x';
        }
        return null;
    };
    ...
    const handleGetEstimate = async () => {
        ...
        try {
            const params: EVMRawTransactionIntentParams = {
                caip2Id: formData.selectedChain,
                transaction: {
                    from: formData.fromAddress as `0x${string}`,
                    to: formData.toAddress as `0x${string}`,
                    data: formData.transactionData as `0x${string}`,
                    value: BigInt(formData.value || '0')
                }
            };

            const result = await evmRawTransactionWithEstimate(
                oktoClient,
                params,
    ...
    return (
        <div className="min-h-screen p-6">
            <div className="max-w-6xl mx-auto">
                ...
                {/* Main Container */}
                <div className="relative bg-gradient-to-br from-white via-blue-50 to-purple-50 rounded-2xl shadow-2xl overflow-hidden border border-slate-200">
                    {/* Glowing Border Effect */}
                    <div className="absolute inset-0 bg-gradient-to-r from-blue-500/10 via-purple-500/10 to-cyan-500/10 blur-sm"></div>
                        ...
                        <div className="relative p-8 pb-6">
                            <div className="flex items-center space-x-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-slate-600 text-lg">Execute custom blockchain transactions with precision</p>
                                </div>
                            </div>
                                ...
                                {[1, 2, 3].map((step) => (
                                    <div key={step} className="flex items-center">
                                        <div className={`
                                            ...
                                            ${currentStep >= step
                                                ? 'bg-gradient-to-br from-blue-500 to-purple-600 border-blue-400 text-white shadow-lg shadow-blue-500/30'
                            ...
                            <div className="p-8 pt-0 space-y-8">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="relative">
                                            <select
                                                ...
                                                className="w-full p-4 bg-white border border-slate-300 rounded-xl text-slate-700 appearance-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 shadow-sm"
                                            >
                                                ...
                                                <div>
                                                    ...
                                                    <p className={`text-sm mt-1 ${
                                                        ...
                                                        {sponsorshipEnabled
                                                            ...
                                                            : 'Native tokens required for transactions. Please obtain tokens from respective faucets.'}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <textarea
                                        ...
                                        placeholder="0x..."
                                        className="w-full p-4 bg-white border border-slate-300 rounded-xl text-slate-700 placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 shadow-sm font-mono text-sm h-32 resize-none"
                                    />
                                </div>
                                ...
                                <div className="bg-slate-50 border border-slate-200 p-6 rounded-xl shadow-sm">
                                    ...
                                    <div className="grid grid-cols-1 gap-4 text-sm">
                                        ...
                                        <div className="flex flex-col space-y-1">
                                            ...
                                            <span className="text-slate-800 font-mono text-xs break-all">{formData.toAddress}</span>
                                        </div>
                                        <div className="flex flex-col space-y-1">
                                            <span className="text-slate-500">Data:</span>
                                            <div className="text-slate-800 font-mono text-xs break-all bg-slate-100 p-3 rounded-lg border">
                                                {formData.transactionData}
                                            </div>
                                        </div>
                                        <div className="flex flex-col space-y-1">
                                            <span className="text-slate-500">Value:</span>
                                            <span className="text-slate-800 font-semibold">{formData.value} wei</span>
                                        </div>
                                    </div>
                                </div>
...
export default EVMRawTransactionEstimate;